...
public class Company extends CreatureBase {
	...
	public Company(Handler handler, float x, float y) {
		...
		animLeft = new Animation(animWalkingSpeed, Images.Company_back);
		animRight = new Animation(animWalkingSpeed, Images.Company_left);
		animUp = new Animation(animWalkingSpeed, Images.Company_right);
	...
	public void render(Graphics g) {
		...
		if(handler.getWorld().getEntityManager().getPlayer().CompanyStatus()== 1) {
			g.drawImage(
					getCurrentAnimationFrame(animDown, animUp, animLeft, animRight, Images.Company_front, Images.Company_back,
							Images.Company_left, Images.Company_right),
					(int) (x - handler.getGameCamera().getxOffset()), (int) (y - handler.getGameCamera().getyOffset()),
					width, height, null);
			if (isBeinghurt() && healthcounter <= 75) {
				g.setColor(Color.white);
				g.drawString("CompanionHealth: " + getHealth(), (int) (x - handler.getGameCamera().getxOffset()),
